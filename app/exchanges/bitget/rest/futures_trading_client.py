...
class BitgetFuturesTradingClient(BitgetRestClient):
    ...
    def get_fills(
        ...
        payload = self.private_request(
            "GET",
            BITGET_FILLS_PATH,
            params={"category": "USDT-FUTURES", "orderId": exchange_order_id},
        )
        return parse_fills(
            payload,
...
def _create_body(request: OrderRequest) -> dict[str, str]:
    body = {
        ...
        "qty": decimal_text(request.quantity),
...
def _request_market(request: OrderRequest) -> None:
    if (
        ...
        raise ValueError("BitgetFuturesTradingClient requires a Bitget USDT perpetual order")
